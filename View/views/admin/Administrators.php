...
<body>
	...
	<main class="d-flex">
		...
		<section class="mt-4 w-75 mx-auto mobile-target">
			<h1 class="fs-3">
				<?php if ($status == 1) echo 'Administradores';
			...
			<section class="table-system mt-3">
				<div class="d-flex justify-content-between gap-1">
					...
					<div class="d-flex justify-content-between gap-1">
						<?php if ($status == 1) { ?>
							<button onclick="CreateAdministrators()" class="btn text-white py-2 table__green-button" title="Crear nuevo"><i class="fa-solid fa-plus"></i></button>
							<button onclick="ChangeStatus(0, 1)" class="btn text-white py-2 table__red-button" title="Eliminar"><i class="fa-solid fa-trash"></i></button>
	...
	<form id="formEdit" action="./?dir=admin&controller=Staff&action=AdminViews&id=modificar" method="post" hidden>
	...
	<div id="data" hidden data-administradores='<?php echo $administrators ?>' data-estadoUsuarios='<?php echo $status ?>'></div>
	<script>
		...
		if (data.dataset.administradores != "") {
			usersArray = JSON.parse(data.dataset.administradores);
			searchInTable("", USER_STATUS);
		}

		function viewActive() {
			location.href = "./?dir=admin&controller=Staff&action=AdminViews&id=main";
		}

		function viewInactive() {
			location.href = "./?dir=admin&controller=Staff&action=AdminViews&id=main&estados=0";
		}

		function CreateAdministrators() {
			location.href = "./?dir=admin&controller=Staff&action=AdminViews&id=crear";
		}

		const formEdit = document.getElementById('formEdit');

		function EditAdministrators(...entityData) {
		...
		function FillInfoModal(...entityData) {
			...
			let status;
			if (entityData[4] === "1") status = "Administrador Activo";
			else status = "Administrador Eliminado";
		...
		function ChangeStatus(status) {
			...
			if (validRoute) {
				let redirect = `./?dir=admin&controller=Staff&action=ChangeStatus&id[]=${status}&id[]=1`;
				redirect += urlIds;
				...
				`
					<tr class="transicion align-middle" data-id="${e.id}">
							...
							<td class="text-center">
								<i class="fa-solid fa-pen-to-square me-1 fs-5" style="cursor: pointer" onclick="EditAdministrators(${e.id}, '${e.nombre}', '${e.apellido1}', '${e.apellido2}', '${e.correo}', '${e.estado}')"></i>
