...
<body>
    ...
    <main class="d-flex">
        ...
        <section class="mt-4 w-75 mx-auto mobile-target">
            <h1 class="fs-3">
                <?php if ($status == 1) echo 'Estudiantes';
            ...
            <section class="table-system mt-3">
                <div class="d-flex justify-content-between gap-1">
                    ...
                    <div class="d-flex justify-content-between gap-1">
                        <?php if ($status == 1) { ?>
                            <button onclick="CreateStudents()" class="btn text-white py-2 table__green-button" title="Crear nuevo"><i class="fa-solid fa-plus"></i></button>
    ...
    <form id="formEdit" action="./?dir=admin&controller=Student&action=Index&id=modificar" method="post" hidden>
    ...
    <div id="data" hidden data-estudiantes='<?php echo $students ?>' data-estadoUsuarios='<?php echo $status ?>'></div>
    ...
    <script>
        ...
        function viewActive() {
            location.href = "./?dir=admin&controller=Student&action=Index&id=main";
        }

        function viewInactive() {
            location.href = "./?dir=admin&controller=Student&action=Index&id=main&estados=0";
        }

        function CreateStudents() {
            location.href = "./?dir=admin&controller=Student&action=Index&id=crear";
        }

        const formEdit = document.getElementById('formEdit');

        function EditStudents(...entityData) {
        ...
        function FillInfoModal(...entityData) {
            ...
            let status;
            if (entityData[9] === "1") status = "Estudiante Activo";
            else status = "Estudiante Eliminado";

            let scholarship;
            if (entityData[7] === "1") scholarship = "Completa";
            else scholarship = "Subvencionada";

            modalTitle.textContent = `${entityData[0]} ${entityData[1]} ${entityData[2]}`;
            removeAllChildNodes(modalBody);
            modalBody.insertAdjacentHTML('beforeend', `
              <img class="text-center" src="${entityData[10]}" alt="Foto de Perfil" width="150" height="150" style="display:block;margin: 0 auto; object-fit: cover; border-radius: 50%">
              <dl>
                <dt>Cédula</dt>
                <dd>${entityData[3]}</dd>
                <dt>Correo</dt>
        ...
        function ChangeStatus(status) {
            ...
            if (validRoute) {
                let redirect = `./?dir=admin&controller=Student&action=ChangeStatus&id=${status}`;
                redirect += urlIds;
        ...
        function searchInTable(text, userStatus) {
            ...
            usersArray.forEach((element) => {
                let found = false;
                let tempArray = Object.values(element).splice(1, 5);
            ...
            tableBodyElement.insertAdjacentHTML(
              ...
              <tr class="transicion align-middle" data-id="${e.id}">
                  ...
                  <td class="text-center">
                    <i class="fa-solid fa-pen-to-square me-1 fs-5" style="cursor: pointer" onclick="EditStudents(${e.id}, '${e.nombre}', '${e.apellido1}', '${e.apellido2}', '${e.cedula}', '${e.especialidad}', '${e.seccion}', '${e.correo}', '${e.estado}', '${e.becado}', '${e.fotoPerfil}')"></i>
                    <i data-bs-toggle="modal" data-bs-target="#exampleModal" class="fa-solid fa-circle-info ms-1 fs-5" style="cursor: pointer" onclick="FillInfoModal('${e.nombre}', '${e.apellido1}', '${e.apellido2}', '${e.cedula}', '${e.correo}', '${e.especialidad}', '${e.seccion}', '${e.becado}', '${e.comidas}', '${e.estado}', '${e.fotoPerfil}')"></i>	
                  </td>	
                </tr>
              `
            );
