...
<body>
	...
	<main class="d-flex">
		...
		<section class="mt-4 w-75 mx-auto mobile-target">
			<h1 class="fs-3">
				<?php if ($status == 1) echo 'Cobradores';
			...
			<section class="table-system mt-3">
				<div class="d-flex justify-content-between gap-1">
					...
					<div class="d-flex justify-content-between gap-1">
						<?php if ($status == 1) { ?>
							<button onclick="CreateBillers()" class="btn text-white py-2 table__green-button" title="Crear nuevo"><i class="fa-solid fa-plus"></i></button>
						...
						<?php } else { ?>
							<button onclick="ChangeStatus(1, 2)" class="btn text-white py-2 table__red-button" title="Activar"><i class="fa-solid fa-heart"></i></i></button>
	...
	<form id="formUpdate" action="./?dir=admin&controller=Staff&action=BillingViews&id=modificar" method="post" hidden>
	...
	<div id="data" hidden data-cobradores='<?php echo $billers ?>' data-estadoUsuarios='<?php echo $status ?>'></div>
	<script>
		...
		if (data.dataset.cobradores != "") {
			usersArray = JSON.parse(data.dataset.cobradores);
			searchInTable("", USER_STATUS);
		}

		function viewActive() {
			location.href = "./?dir=admin&controller=Staff&action=BillingViews&id=main";
		}

		function viewInactive() {
			location.href = "./?dir=admin&controller=Staff&action=BillingViews&id=main&estados=0";
		}

		function CreateBillers() {
			location.href = "./?dir=admin&controller=Staff&action=BillingViews&id=crear";
		}

		const formUpdate = document.getElementById('formUpdate');

		function UpdateBillers(...entityData) {
		...
		function FillInfoModal(...entityData) {
			...
			let status;
			if (entityData[4] === "1") status = "Cobrador Activo";
			else status = "Cobrador Eliminado";
		...
		function ChangeStatus(status, profile) {
			let urlIds = "";
			let lengthArray = 0;
			...
			if (validRoute) {
				let redirect = `./?dir=admin&controller=Staff&action=ChangeStatus&id[]=${status}&id[]=${profile}`;
				redirect += urlIds;
		...
		function searchInTable(text, userStatus) {
			...
			usersArray.forEach((element) => {
				let found = false;
				let tempArray = Object.values(element).splice(1, 3);
				...
				`
					<tr class="transicion align-middle" data-id="${e.id}">
							...
							<td class="text-center">
								<i class="fa-solid fa-pen-to-square me-1 fs-5" style="cursor: pointer" onclick="UpdateBillers(${e.id}, '${e.nombre}', '${e.apellido1}', '${e.apellido2}', '${e.correo}', '${e.estado}')"></i>
								<i data-bs-toggle="modal" data-bs-target="#exampleModal" class="fa-solid fa-circle-info ms-1 fs-5" style="cursor: pointer" onclick="FillInfoModal('${e.nombre}', '${e.apellido1}', '${e.apellido2}', '${e.correo}', '${e.estado}')"></i>	
							</td>	
						</tr>
					`
			);
