...
<body>
		...
		<section class="mt-4 w-75 mx-auto mobile-target">
			...
			<section class="table-system mt-3">
				<div class="d-flex justify-content-between gap-1">
					...
					<div class="d-flex justify-content-between gap-1">
						<button onclick="CreateSpecialties()" class="btn text-white py-2 table__green-button" title="Crear nuevo"><i class="fa-solid fa-plus"></i></button>
	...
	<div id="data" hidden data-especialidades='<?php echo $specialties ?>'></div>

	<form id="formUpdate" action="./?dir=admin&controller=Specialties&action=Index&id=modificar" method="post" hidden>
	...
	<script>
		...
		let usersArray = JSON.parse(data.dataset.especialidades);
		searchInTable("", ELEMENT_STATUS);

		function CreateSpecialties() {
			location.href = "./?dir=admin&controller=Specialties&action=Index&id=crear";
		}

		const formUpdate = document.getElementById('formUpdate');

		function UpdateSpecialties(...entityData) {
		...
		function ChangeStatus(status) {
			...
			if (validRoute) {
				let redirect = `./?dir=admin&controller=Specialties&action=ChangeStatus&id=${status}`;
				redirect += urlIds;
				...
				`
					<tr class="transicion align-middle" data-id="${e.id}">
							...
							<td class="text-center">
								<i class="fa-solid fa-pen-to-square me-1 fs-5" style="cursor: pointer" onclick="UpdateSpecialties(${e.id}, '${e.descripcion}', '${e.estado}')"></i>
							</td>	
						</tr>
					`
			);
